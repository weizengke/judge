/*
	...
	Î´À´·Ö²¼Ê½²¿ÊðË¼Â·
	1) clientÉÏÏßºó£¬serverÎ¬»¤clientÐÅÏ¢(ip,socket,ÅÐÌâÄÜÁ¦)
	2) server´ò°ü·¢ËÍdataÎÄ¼þ¸øclient(clientÉÏÏßÊÇ´¥·¢¡¢dataÄ¿Â¼ÎÄ¼þ±ä»¯Ê±´¥·¢)
	3) clientÏÂÏßºóÍ¨Öªserver
	4) server¶¨Ê±ping client ÐÄÌøÎ¬³Ö£¬Èô³¤Ê±¼äÎÞ»ØÓ¦ÔòÉ¾³ýclient
...
queue <JUDGE_DATA_S> g_JudgeQueue; /* È«¾Ö¶ÓÁÐ */
...
void Judge_InitJudgePath(JUDGE_SUBMISSION_ST *pstJudgeSubmission)
{
	...
	strcpy(pstJudgeSubmission->compileCmd,compile_string.c_str());       /* ±àÒëÃüÁîÐÐ */
	...
	strcpy(pstJudgeSubmission->runCmd,runcmd_string.c_str());			 /* ÔËÐÐÃüÁîÐÐ*/
	...
	strcpy(pstJudgeSubmission->sourcePath,sourcepath_string.c_str());		 /* Ô´³ÌÐòÂ·¾¶*/
	...
	strcpy(pstJudgeSubmission->exePath,exepath_string.c_str());				 /* ¿ÉÖ´ÐÐÎÄ¼þÂ·¾¶*/

	sprintf(pstJudgeSubmission->DebugFile,"%s%s%s.txt",workPath,pstJudgeSubmission->subPath,name.c_str());  /* debugÎÄ¼þÂ·¾¶*/
	sprintf(pstJudgeSubmission->ErrorFile,"%s%s%s_re.txt",workPath,pstJudgeSubmission->subPath,name.c_str());  /* reÎÄ¼þÂ·¾¶*/
...
int Judge_RunLocalSolution(JUDGE_SUBMISSION_ST *pstJudgeSubmission)
{
	...
	for(i=1; ;++i)
	{
		...
		DWORD status_ = WaitForSingleObject(hThread_run, pstJudgeSubmission->stProblem.time_limit + 1000);   /* ·Å¿íÊ±ÏÞ1000mS,·µ»ØÖµ´óÓÚÁãËµÃ÷³¬Ê±. */
		if(status_>0)
		{
...
int Judge_Local(JUDGE_SUBMISSION_ST *pstJudgeSubmission)
{
	...
	else
	{
        ...
        {
			/* Ò»·ÖÖÓÄÚÃ»ÓÐÌá½»£¬ÖØ¸´Ò»±é£¬¹æ±Ü³¤Ê±¼äÃ»ÓÐjudgeµ¼ÖÂµÄÔËÐÐÊ±¼ä²»×¼ÎÊÌâ */
            time_t s_t = 0;            
            time(&s_t);
	...
	do
	{
		if (0 == strcmp(pstJudgeSubmission->stProblem.szVirJudgerName,"HDU"))
		{
			...
			if (OS_YES == hdu_remote_enable)
			{
				...
				if (OS_OK == ret)
				{
					/* virdictÖÃqueue , ÓÉÔ¶³Ìjudger¼ÌÐøÖ´ÐÐ */
		...
		if (0 == strcmp(pstJudgeSubmission->stProblem.szVirJudgerName,"GUET_DEPT3"))
		{
			...
			if (OS_YES == guet_remote_enable)
			{
				...
				if (OS_OK == ret)
				{
					/* virdictÖÃquieue , ÓÉÔ¶³Ìjudger¼ÌÐøÖ´ÐÐ */
					pstJudgeSubmission->stSolution.verdictId = V_Q;
				}
...
unsigned _stdcall  Judge_Proc(void *pData)
{
	...
	if (OS_YES == stJudgeSubmission.stProblem.isVirtualJudge)
	{
		...
		stJudgeSubmission.stSolution.testcase = 0;  /* ºóÐø¿ÉÄÜ²»Ìî0 */
...
/* virtual-judge & local-judge Ó¦·ÖÁ½¸ö¶ÓÁÐ */
