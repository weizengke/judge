/*
	...
	Œ¥¿¥∑÷≤º Ω≤ø Àº¬∑
	1) client…œœﬂ∫Û£¨serverŒ¨ª§client–≈œ¢(ip,socket,≈–Ã‚ƒ‹¡¶)
	2) server¥Ú∞¸∑¢ÀÕdataŒƒº˛∏¯client(client…œœﬂ «¥•∑¢°¢dataƒø¬ºŒƒº˛±‰ªØ ±¥•∑¢)
	3) clientœ¬œﬂ∫ÛÕ®÷™server
	4) server∂® ±ping client –ƒÃ¯Œ¨≥÷£¨»Ù≥§ ±º‰Œﬁªÿ”¶‘Ú…æ≥˝client
...
queue <JUDGE_DATA_S> g_JudgeQueue; /* »´æ÷∂”¡– */
...
void Judge_InitJudgePath(JUDGE_SUBMISSION_ST *pstJudgeSubmission)
{
	...
	strcpy(pstJudgeSubmission->compileCmd,compile_string.c_str());       /* ±‡“Î√¸¡Ó–– */
	...
	strcpy(pstJudgeSubmission->runCmd,runcmd_string.c_str());			 /* ‘À––√¸¡Ó––*/
	...
	strcpy(pstJudgeSubmission->sourcePath,sourcepath_string.c_str());		 /* ‘¥≥Ã–Ú¬∑æ∂*/
	...
	strcpy(pstJudgeSubmission->exePath,exepath_string.c_str());				 /* ø…÷¥––Œƒº˛¬∑æ∂*/

	sprintf(pstJudgeSubmission->DebugFile,"%s%s%s.txt",workPath,pstJudgeSubmission->subPath,name.c_str());  /* debugŒƒº˛¬∑æ∂*/
	sprintf(pstJudgeSubmission->ErrorFile,"%s%s%s_re.txt",workPath,pstJudgeSubmission->subPath,name.c_str());  /* reŒƒº˛¬∑æ∂*/
...
unsigned _stdcall Judge_RunProgramThread(void *pData) //ac
{
	...
	/* ChildIn_Write «◊”Ω¯≥Ãµƒ ‰»Îæ‰±˙£¨ChildIn_Read «∏∏Ω¯≥Ã”√”⁄–¥»Î◊”Ω¯≥Ã ‰»Îµƒæ‰±˙ */
	HANDLE ChildIn_Read, ChildIn_Write;
	/*ChildOut_Write «◊”Ω¯≥Ãµƒ ‰≥ˆæ‰±˙£¨ChildOut_Read «∏∏Ω¯≥Ã”√”⁄∂¡»°◊”Ω¯≥Ã ‰≥ˆµƒæ‰±˙*/
...
int Judge_RunLocalSolution(JUDGE_SUBMISSION_ST *pstJudgeSubmission)
{
	...
	for(i=1; ;++i)
	{
		...
		DWORD status_ = WaitForSingleObject(hThread_run, pstJudgeSubmission->stProblem.time_limit + 10);   /* ∑≈øÌ ±œﬁ10mS,∑µªÿ÷µ¥Û”⁄¡„Àµ√˜≥¨ ±. */
		if(status_>0)
		{
...
int Judge_Local(JUDGE_SUBMISSION_ST *pstJudgeSubmission)
{
	...
	else
	{
        ...
        {
            /* Ë∑ùÁ¶ª‰∏ä‰∏ÄÊ¨°Âà§È¢òË∂ÖËøá60sÔºåÈúÄË¶ÅÂÜçÊâßË°å‰∏ÄÈÅç */
            time_t s_t = 0;            
            time(&s_t);
...
unsigned _stdcall  Judge_Proc(void *pData)
{
	...
	if (OS_YES == stJudgeSubmission.stProblem.isVirtualJudge)
	{
		...
		stJudgeSubmission.stSolution.testcase = 0;  /* ∫Û–¯ø…ƒ‹≤ªÃÓ0 */
...
//µ⁄1∏ˆ≤Œ ˝£∫∞≤»´ Ù–‘£¨NULLŒ™ƒ¨»œ∞≤»´ Ù–‘
//µ⁄2∏ˆ≤Œ ˝£∫÷∏∂®œﬂ≥Ã∂—’ªµƒ¥Û–°°£»Áπ˚Œ™0£¨‘Úœﬂ≥Ã∂—’ª¥Û–°∫Õ¥¥Ω®À¸µƒœﬂ≥Ãµƒœ‡Õ¨°£“ª∞„”√0
//µ⁄3∏ˆ≤Œ ˝£∫÷∏∂®œﬂ≥Ã∫Ø ˝µƒµÿ÷∑£¨“≤æÕ «œﬂ≥Ãµ˜”√÷¥––µƒ∫Ø ˝µÿ÷∑(”√∫Ø ˝√˚≥∆º¥ø…£¨∫Ø ˝√˚≥∆æÕ±Ì æµÿ÷∑)
//µ⁄4∏ˆ≤Œ ˝£∫¥´µ›∏¯œﬂ≥Ãµƒ≤Œ ˝µƒ÷∏’Î£¨ø…“‘Õ®π˝¥´»Î∂‘œÛµƒ÷∏’Î£¨‘⁄œﬂ≥Ã∫Ø ˝÷–‘Ÿ◊™ªØŒ™∂‘”¶¿‡µƒ÷∏’Î
//µ⁄5∏ˆ≤Œ ˝£∫œﬂ≥Ã≥ı º◊¥Ã¨£¨0:¡¢º¥‘À––£ªCREATE_SUSPEND£∫suspended£®–¸π“£©
//µ⁄6∏ˆ≤Œ ˝£∫”√”⁄º«¬ºœﬂ≥ÃIDµƒµÿ÷∑
