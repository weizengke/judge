...
#define JUDGE_RECENT_TIMEOUT  300
...
char g_judge_proxy_url[128] = {0};
char g_judge_proxy_uname_passwd[128] = {0};

void judge_set_proxy(CURL *curl)
{
	if (strlen(g_judge_proxy_url) == 0) {
		return ;
	}

	curl_easy_setopt(curl, CURLOPT_PROXY, g_judge_proxy_url);
	curl_easy_setopt(curl, CURLOPT_PROXYUSERPWD, g_judge_proxy_uname_passwd);
...
int judge_get_contest_zoj(vector<contest_t> &contest_v)
{
    ...
    if(curl)
    {
		...
		curl_easy_setopt(curl, CURLOPT_TIMEOUT, JUDGE_RECENT_TIMEOUT);
		judge_set_proxy(curl);
...
int judge_parse_contest_info_codechef(char *html, contest_t * contest)
{
	string string_ = html;
	...
	Judge_Debug(DEBUG_TYPE_FUNC, "string_=%s.", string_.c_str());

	{
		...
		if (-1 == pos)
		{
			printf("\r\nError: Cound not found codechef url.");
			return 1;
		}
		pos += strlen("<a href=\"");
		...
		if (-1 == pos2)
		{
			printf("\r\nError: Cound not found codechef url 2.");
			return 1;
		}

		string ts = s.substr(pos, pos2 - pos);

		contest->url = "https://www.codechef.com" + ts;
...

	{
		...
		if (-1 == pos)
		{
			printf("\r\nError: Cound not found codechef title 2. string_=%s.", string_.c_str());
			return 1;
		}
...

	{
		...
		int pos =s.find("data-starttime=\"");
		if (-1 == pos)
		{
			printf("\r\nError: Cound not found codechef start time 1.");
			return 1;
		}
		pos += strlen("data-starttime=\"");

		int pos2 =s.find("\" class=\"start_date\">");
		if (-1 == pos2)
		{
			printf("\r\nError: Cound not found codechef start time 2.");
			return 1;
		}

		string ts = s.substr(pos, pos2 - pos);

		int day, month, year, hour, min_, sec, pp, mm = 0;

		//2020-11-22T21:30:00+05:30
		sscanf(ts.c_str(), "%d-%d-%dT%d:%d:%u+%d:%d",
				&year, &month, &day, &hour, &min_, &sec, &pp, &mm);
	...
	contest->oj_name = "CodeChef";
	contest->public_b = true;

	return 0;
}

int judge_get_contest_codechef(vector<contest_t> &contest_v)
{
	FILE *fp = fopen(g_judge_recent_tmpfile, "wb+");
	CURL *curl;
	CURLcode res;

	Judge_Debug(DEBUG_TYPE_FUNC, "judge_get_contest_codechef start...");
    ...
    if(curl)
    {
        ...
        char url[255] = {0};
        sprintf(url, "https://www.codechef.com/contests");
        curl_easy_setopt(curl, CURLOPT_URL, url);
		curl_easy_setopt(curl, CURLOPT_WRITEFUNCTION, &judge_write_cb);
		curl_easy_setopt(curl, CURLOPT_WRITEDATA, fp);
        ...
        Judge_Debug(DEBUG_TYPE_FUNC, "judge_get_contest_codechef, post request to codechef. (url=%s, res=%d)",
                        url, res);
	...
	int pos =ts_.find("Future Contests</h3>");
	if (-1 == pos)
	{
		Judge_Debug(DEBUG_TYPE_FUNC, "Error: codechef Cound not found upcoming contests.");
		return 1;
	}
	pos += strlen("Future Contests</h3>");

	int pos2 =ts_.find("Past Contests</h3>");
	if (-1 == pos)
	{
		Judge_Debug(DEBUG_TYPE_FUNC, "Error: codechef Cound not found Contest history.");
		return 1;
	}
	...
	Judge_Debug(DEBUG_TYPE_FUNC, "codechef find size1=%u", res1.size());

	for(int i=0; i<res1.size(); i++)
	{
		Judge_Debug(DEBUG_TYPE_FUNC, "codechef find size2=%u", res1[i].value.size());
		...
		for(int j = 0; j<res1[i].value.size(); j++)
		{
			...
			int ret = judge_parse_contest_info_codechef((char*)res1[i].value[j].c_str(), &contest);
			if (0 != ret)
			{
				continue;
			}
			...
			string time_s = "";
			(VOID)util_time_to_string(time_s, contest.start_t);
			Judge_Debug(DEBUG_TYPE_FUNC, "codechef:\r\nOJ:%s\r\nTitle:(%s)\r\nurl=%s\r\ntime:%s\r\n%s\r\n",
					contest.oj_name.c_str(),
					contest.title.c_str(),
					contest.url.c_str(),
					time_s.c_str(),
...
vector<contest_t> contest_v_codechef;
...
int judge_recent_generate(void *p)
{
	...
	{
		Judge_Debug(DEBUG_TYPE_FUNC, "Collect recent contests ,but now is running");
		//return 0;
	}
	...
	Judge_Debug(DEBUG_TYPE_FUNC, "Collect recent contests start. (json_path=%s)", g_judge_recent_json_path);
	...
	if (0 == ret)
	{
		...
		Judge_Debug(DEBUG_TYPE_FUNC, "Collect zoj-contests ok. (size=%d)", contest_v_.size());
	}
	else
	{
		Judge_Debug(DEBUG_TYPE_FUNC, "Collect zoj-contests failed.");
	}
	...
	if (0 == ret)
	{
		...
		Judge_Debug(DEBUG_TYPE_FUNC, "Collect hdu-contests ok. (size=%d)", contest_v_.size());
	}
	else
	{
		Judge_Debug(DEBUG_TYPE_FUNC, "Collect hdu-contests failed.");
	}
	...
	if (0 == ret)
	{
		...
		Judge_Debug(DEBUG_TYPE_FUNC, "Collect codeforces-collect ok. (size=%d)", contest_v_.size());
	}
	else
	{
		Judge_Debug(DEBUG_TYPE_FUNC, "Collect codeforces-contests failed.");
	}
	...
	if (0 == ret)
	{
		...
		Judge_Debug(DEBUG_TYPE_FUNC, "Collect atcoder-contests ok. (size=%d)", contest_v_.size());
	}
	else
	{
		Judge_Debug(DEBUG_TYPE_FUNC, "Collect atcoder-contests failed.");
	}
	...
	if (0 == ret)
	{
		...
		Judge_Debug(DEBUG_TYPE_FUNC, "Collect hihocoder-contests ok. (size=%d)", contest_v_.size());
	}
	else
	{
		Judge_Debug(DEBUG_TYPE_FUNC, "Collect hihocoder-contests failed.");
	}
	contest_v.insert(contest_v.end(), contest_v_hihocoder.begin(), contest_v_hihocoder.end());

	contest_v_.clear();
	ret = judge_get_contest_codechef(contest_v_);
	if (0 == ret)
	{
		contest_v_codechef.clear();
		contest_v_codechef.insert(contest_v_codechef.end(), contest_v_.begin(), contest_v_.end());
		Judge_Debug(DEBUG_TYPE_FUNC, "Collect codechef-contests ok. (size=%d)", contest_v_.size());
	}
	else
	{
		Judge_Debug(DEBUG_TYPE_FUNC, "Collect codechef-contests failed.");
	}
	contest_v.insert(contest_v.end(), contest_v_codechef.begin(), contest_v_codechef.end());
	...
	Judge_Debug(DEBUG_TYPE_FUNC, "Collect recent contests ok. (json_path=%s)", g_judge_recent_json_path);
