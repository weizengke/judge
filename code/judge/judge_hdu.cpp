...
int getProblemInfo_Brief(string pid)
{
	...
	char  patternMemory [] = "(\\d*) K"; 

	char  patternTitle [] = "<h1 style='color:#1A5CC8'>([\\s\\S]*?)</h1>";  

	char  patternDescription [] = "Problem Description</div> <div class=panel_content>([\\s\\S]*?)</div><div class=panel_bottom>&nbsp;</div>"; 

	char  patternInput [] = "Input</div> <div class=panel_content>([\\s\\S]*?)</div><div class=panel_bottom>&nbsp;</div>"; 

	char  patternOutput [] = "Output</div> <div class=panel_content>([\\s\\S]*?)</div><div class=panel_bottom>&nbsp;</div>"; 

	char  patternSampleInput [] = "Sample Input</div><div class=panel_content><pre><div style=\"font-family:Courier New,Courier,monospace;\">([\\s\\S]*?)</div></pre></div><div class=panel_bottom>&nbsp;</div>";  

	char  patternSampleOutput [] = "Sample Output</div><div class=panel_content><pre><div style=\"font-family:Courier New,Courier,monospace;\">([\\s\\S]*?)</div></pre></div><div class=panel_bottom>&nbsp;</div>"; 

	char  patternAuthor [] = "Author</div> <div class=panel_content>([\\s\\S]*?)</div><div class=panel_bottom>&nbsp;</div>";  
...
int Judge_Via_CurlLib(JUDGE_SUBMISSION_S *pstJudgeSubmission)
{
	int ret = OS_TRUE;
	...
	sprintf(tmp_return_path, "%s//OJ_TMP//hdujudge-%d.tmp",current_path,pstJudgeSubmission->solution.solutionId);
	do
	{
		...
		string source_ = "";
		int lang_id = getHDULangID(pstJudgeSubmission->solution.languageId);
		...
		ret = DLL_HDUSubmit(pstJudgeSubmission->problem.virtualPID, lang_id, source_);
		if (OS_TRUE != ret)
		{
		...
		while (ret != OS_TRUE)
		{
			...
			ret =DLL_HDUGetStatus(hdu_username, pstJudgeSubmission->problem.virtualPID, lang_id, runid, result,ce_info,tu,mu);
			if (ret != OS_TRUE
		...
		if (OS_FALSE == ret)
		{
			...
			pstJudgeSubmission->solution.verdictId = V_SK;
		}
		else
		{
			MSG_OUPUT_DBG("Get Status success...");
...
int HDU_VJudge(JUDGE_SUBMISSION_S *pstJudgeSubmission)
{
	int ret = 0;
